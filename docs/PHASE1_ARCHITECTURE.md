...
3. The runtime uses the Claude Agent SDK for planning, execution, and verification, but state transitions remain in local code.
4. Runtime and broker events append to `.harness/.../events/events.ndjson`.
5. Session and run state snapshots persist in `session.json` and `run.json` for resume.
